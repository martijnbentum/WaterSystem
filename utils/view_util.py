...
class Change:
    def __init__(self, user, time, field_name, state, related_name, related_instance=None):
        ...
        elif not related_instance or not related_name:
...
def make_tabs(tab_type, focus=0, focus_names=''):
    ...
    if tab_type == 'person':
        t = 'Persons, Citys, Neighbourhoods, Institutions, Installations, Evidences'
    ...
    if tab_type == 'institution':
        t = 'Institutions, Persons,Installations,Evidences'
        relations = Tab(t, focus)
        return Tabs([minimize, relations], 'minimize,relations', focus_names)
